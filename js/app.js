...
function bindNavigation(){
  $$('.nav button,.mobile-nav button,[data-go]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const target=btn.dataset.view||btn.dataset.go;
      if(target) location.hash=target;
      $('.mobile-nav').classList.remove('open');
    });
  });
  $('.menu-btn').onclick=()=>$('.mobile-nav').classList.toggle('open');
}
function route(){
  const view=(location.hash||'#home').slice(1);
  state.currentView=['home','music','videos','experiences','support'].includes(view)?view:'home';
  $$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${state.currentView}`));
  $$('.nav button,.mobile-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===state.currentView));
