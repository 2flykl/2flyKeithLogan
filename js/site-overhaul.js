...
function renderPlayables(){
  ...
  const render=filter=>{const list=app.playables.filter(p=>filter==='all'||p.status===filter||p.category===filter);$('#playablesGrid').innerHTML=list.map(playableCard).join('')||'<p class="empty">No Playables match this filter yet.</p>'};
