/* 2Fly Public Preview — Playable Experiences release layer */
(() => {
  'use strict';
  ...
  const postedKey = '2fly-keep-me-posted-preview-v1';
  ...
  if (originalPlayProject) {
    playProject = function publicPreviewPlayProject(project, openMusic = true) {
  ...
  if (originalOpenVideo) {
    openVideo = function publicPreviewOpenVideo(project) {
  ...
  function bindStaticCreateActions() {
    ...
    qs('#postedForm')?.addEventListener('submit', event => {
      ...
      showToast('Your Keep Me Posted preferences are saved on this device.');
    });
...
/* Public preview feedback and early-visitor tools */
(function(){
  ...
  document.querySelectorAll('[data-feedback-open]').forEach(button=>button.addEventListener('click',()=>{
    feedbackOverlay?.classList.add('open');document.body.classList.add('locked');
  }));
  feedbackOverlay?.querySelector('.overlay-close')?.addEventListener('click',()=>{
    feedbackOverlay.classList.remove('open');document.body.classList.remove('locked');
  });
  ...
  form?.addEventListener('submit',async event=>{
    ...
    const existing=JSON.parse(localStorage.getItem('2fly-public-feedback')||'[]');
    existing.push(data);localStorage.setItem('2fly-public-feedback',JSON.stringify(existing));
    const message=`2Fly Website Feedback\n\nExplored: ${data.area}\nWhat stood out: ${data.favorite}\nWhat should be fixed: ${data.improve}\nUnderstood Playable Experiences: ${data.understood}\nName: ${data.name||'Not provided'}\nEmail: ${data.email||'Not provided'}\nContact permission: ${data.contactPermission?'Yes':'No'}`;
    try{await navigator.clipboard.writeText(message);showToast('Feedback saved and copied. Thank you.')}catch(_){showToast('Feedback saved. Thank you.')}
