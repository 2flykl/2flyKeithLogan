...
function catchBoom(g,perfect=true){g.playerX=g.landingX;while(g.flightTime/g.duration<(perfect?.97:.91))g.step(1/240);g.actionDown();tick(g,.8);}
test('Charge releases once and owns a single projectile',()=>{const g=new ReturnGame();throwBoom(g);g.actionUp();assert.equal(g.attempts,1);assert.equal(g.phase,'flight');assert.ok(g.spin>.88)});
...
test('Wider return zone accepts a player within the expanded catch radius',()=>{const g=new ReturnGame();throwBoom(g);while(g.flightTime/g.duration<.97)g.step(.01);g.playerX=g.landingX+.75;g.actionDown();tick(g,.9);assert.equal(g.returns,1)});
