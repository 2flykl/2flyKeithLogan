...
for(const name of ['index.html','style.css','photo-reel.css','src','vendor','assets'])fs.cpSync(path.join(root,name),path.join(output,name),{recursive:true,filter:p=>{const rel=path.relative(root,p).split(path.sep).join('/');return !/(^|\/)(proof|masters|references)(\/|$)/.test(rel)&&!rel.endsWith('-review.jpg')&&!(rel.startsWith('assets/characters/')&&rel.endsWith('.png'));}});
