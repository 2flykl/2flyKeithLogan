...
const unique=new Set(Object.values(library.phases).flat().map(x=>x.src));
...
for(const shots of Object.values(library.phases)){if(shots.length!==3)blockers.push('Each phase must have exactly three photographs.');for(const shot of shots)if(!fs.existsSync(path.join(root,shot.src)))blockers.push('Missing media: '+shot.src);}
if(library.status!=='release-candidate')blockers.push('Media library has not passed final art review.');
