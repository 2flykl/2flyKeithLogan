...
for(const [id,key] of [['left','a'],['right','d']]){const el=$(id);el.addEventListener('pointerdown',e=>{e.preventDefault();if(blocked())return;el.setPointerCapture(e.pointerId);keys.add(key);});for(const evt of ['pointerup','pointercancel','lostpointercapture'])el.addEventListener(evt,()=>keys.delete(key));}
...
function updateUI(){
 ...
 const point=world.project(game.landingX,.035,game.landingZ);const catchRadius=game.catchRadius??.96;show('landing',flying&&point.visible&&!exploring&&!reel.active);$('landing').style.left=point.x+'px';$('landing').style.top=point.y+'px';$('landing').style.opacity=Math.abs(game.playerX-game.landingX)<catchRadius?'1':'.86';$('landing').classList.toggle('aligned',Math.abs(game.playerX-game.landingX)<catchRadius);if(!flying)$('range').textContent='A little patience goes a long way.';
}
...
try{
 ...
 function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;
  ...
  if(started&&!complete)updateUI();if(now>noticeUntil)$('notice').classList.remove('show');
