...
export interface EraOrbitHit { galaxyId:string; title:string; worldPos:THREE.Vector3; subtitle:string; }
type Kind='planet'|'moon'|'artifact'|'disc'|'crystal';
interface ShellDef { title:string; subtitle:string; texture?:string; radius:number; orbit:number; speed:number; kind:Kind; }
...
export class EraOrbitSystem {
  readonly group=new THREE.Group(); readonly clickTargets:THREE.Object3D[]=[];
  private orbiters:{pivot:THREE.Group;body:THREE.Mesh;speed:number;bob:number}[]=[];
  private loader=new THREE.TextureLoader();
  constructor(public readonly galaxyId:string){
    const theme=GALAXY_THEMES[galaxyId],defs=ERA_SHELLS[galaxyId]; if(!theme||!defs)return;
    this.group.position.set(...theme.worldOffset);
    defs.forEach((def,i)=>{
      const pivot=new THREE.Group(); pivot.rotation.x=(i-1.4)*.18; pivot.rotation.z=i*.47+.12;
      const geo=this.geometryFor(def);
      const mat=this.materialFor(def,theme);
      const body=new THREE.Mesh(geo,mat); body.position.x=def.orbit;
      body.userData={eraShell:true,galaxyId,title:def.title,subtitle:def.subtitle};
      pivot.add(body);this.group.add(pivot);this.clickTargets.push(body);this.orbiters.push({pivot,body,speed:def.speed,bob:i*.8});
      this.addOrbitTrail(pivot,def,theme,i);
      this.decorate(body,def,theme,i);
    });
    const light=new THREE.PointLight(theme.accentColor,1.8,15000);this.group.add(light);
  }
  private geometryFor(def:ShellDef){
    if(def.kind==='artifact')return new THREE.DodecahedronGeometry(def.radius,1);
    if(def.kind==='crystal')return new THREE.IcosahedronGeometry(def.radius,2);
    if(def.kind==='disc')return new THREE.CylinderGeometry(def.radius,def.radius,Math.max(42,def.radius*.16),64);
    return new THREE.SphereGeometry(def.radius,48,32);
  }
  private materialFor(def:ShellDef,theme:any):THREE.MeshStandardMaterial{
    if(def.texture){const tex=this.loader.load(def.texture);tex.colorSpace=THREE.SRGBColorSpace;tex.wrapS=THREE.RepeatWrapping;return new THREE.MeshStandardMaterial({map:tex,roughness:def.kind==='moon'?.72:.42,metalness:def.kind==='planet'?.12:.3,emissive:theme.nebulaColor,emissiveIntensity:.2});}
  ...
  private addOrbitTrail(pivot:THREE.Group,def:ShellDef,theme:any,i:number){
    ...
    const line=new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints(pts),new THREE.LineBasicMaterial({color:theme.accentColor,transparent:true,opacity:.055+i*.008,depthWrite:false}));line.rotation.x=Math.PI/2;pivot.add(line);
  }
  private decorate(body:THREE.Mesh,def:ShellDef,theme:any,i:number){
    if(def.kind==='planet'||def.kind==='moon'){
      const ring=new THREE.Mesh(new THREE.TorusGeometry(def.radius*1.38,16+i*4,10,100),new THREE.MeshBasicMaterial({color:theme.accentColor,transparent:true,opacity:.15,depthWrite:false,blending:THREE.AdditiveBlending}));ring.rotation.x=Math.PI/(2.2+i*.08);body.add(ring);
    }
    if(def.kind==='disc'){
      body.rotation.z=Math.PI/2; const label=new THREE.Mesh(new THREE.CylinderGeometry(def.radius*.38,def.radius*.38,def.radius*.18+4,48),new THREE.MeshStandardMaterial({color:theme.primaryColor,metalness:.35,roughness:.28,emissive:theme.nebulaColor,emissiveIntensity:.25})); body.add(label);
    }
    if(def.kind==='artifact'||def.kind==='crystal'){
      const halo=new THREE.Mesh(new THREE.TorusKnotGeometry(def.radius*1.45,18,90,8,2,3),new THREE.MeshBasicMaterial({color:theme.starTint,transparent:true,opacity:.20,depthWrite:false,blending:THREE.AdditiveBlending}));body.add(halo);
    }
  }
  getHit(raycaster:THREE.Raycaster):EraOrbitHit|null{const hits=raycaster.intersectObjects(this.clickTargets,false);if(!hits.length)return null;const obj=hits[0].object,wp=new THREE.Vector3();obj.getWorldPosition(wp);return{galaxyId:this.galaxyId,title:String(obj.userData.title||'ARCHIVE OBJECT'),subtitle:String(obj.userData.subtitle||'Historical archive destination'),worldPos:wp};}
  getTourStops():TourStop[]{return this.clickTargets.map((obj,i)=>{const wp=new THREE.Vector3();obj.getWorldPosition(wp);return{id:`${this.galaxyId}-ARCHIVE-${i}`,name:String(obj.userData.title),subtitle:`${GALAXY_THEMES[this.galaxyId]?.title} · ${String(obj.userData.subtitle||'Archive destination')}`,pos:{x:wp.x,y:wp.y,z:wp.z}};});}
  update(dt:number){for(const o of this.orbiters){o.pivot.rotation.y+=dt*o.speed;o.body.rotation.y+=dt*.14;o.body.rotation.x+=dt*.035;o.body.position.y=Math.sin(performance.now()*.00035+o.bob)*90;}}
}
