...
export function getChildTexturePath(child: { mediaKind?: string; title?: string; id?: string }): string {
  ...
  if (mk === 'audio') {
    if (title.includes('lyrics')) return 'assets/object_styles/lyrics_moon.png';
    if (title.includes('stem') || title.includes('instrumental')) return 'assets/object_styles/stem_moon.png';
...
export interface DecoratedObjectResult {
  group: THREE.Group;
  clickTarget: THREE.Mesh;
}

export function createDecoratedPlanet(
  objectId: string,
  size: number,
  accentColorHex: number
): DecoratedObjectResult {
...
export function createDecoratedChild(
  childData: { id: string; mediaKind?: string; title?: string },
  size: number,
  accentColorHex: number
): DecoratedObjectResult {
  const group = new THREE.Group();
