...
import { getAllGalaxies, getGalaxyRegions, getAllCelestialObjects, formatAU } from '../data/universe-data';

export type NavigatorTab = 'map' | 'legend';

export interface NavigatorCallbacks {
  onTravelToGalaxy: (galaxyId: string) => void;
  onTravelToRegion: (galaxyId: string, regionId: string) => void;
  onTravelToObject: (objectId: string) => void;
}
...
export class GalacticNavigator {
  ...
  private activeTab: NavigatorTab = 'map';
  private isOpen = false;
  private callbacks: NavigatorCallbacks;

  constructor(container: HTMLElement, callbacks: NavigatorCallbacks) {
    ...
    store.on(() => {
      if (this.isOpen) this._updateTelemetry();
    });
  }
  ...
  render() {
    ...
    this.panel.querySelectorAll('.nav-tab-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const tab = (e.currentTarget as HTMLElement).dataset['tab'] as NavigatorTab;
        this.activeTab = tab;
        this.render();
      });
    });
  ...
  private _renderLegendHTML(): string {
    const legendItems = [
