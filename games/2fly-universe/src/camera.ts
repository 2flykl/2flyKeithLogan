...
import { GALAXY_THEMES, UNIVERSE_HOME_CAMERA } from './types';
...
export class UniverseCamera {
  ...
  private leftDownAt = new THREE.Vector2();
  ...
  private thrustDirection = new THREE.Vector3();
  ...
  private _bindEvents(canvas: HTMLElement) {
    ...
    canvas.addEventListener('mousemove', e => {
      ...
      if (Math.hypot(e.clientX - this.leftDownAt.x, e.clientY - this.leftDownAt.y) > 6) this.suppressNextLeftClick = true;
    ...
    window.addEventListener('mouseup', e => {
      ...
      if (e.button === 2 && this.thrusting) this._stopThrust(false);
    });
    ...
    window.addEventListener('keydown', e => { if (e.key === 'Escape') window.dispatchEvent(new CustomEvent('universe-esc')); });
  }

  private _setGalaxyOrbitPivotIfInside() {
    ...
    for (const [id, theme] of Object.entries(GALAXY_THEMES)) {
      ...
      if (d < enterRadius && d < bestDist) {
        bestId = id; bestDist = d; bestCenter = center;
      }
    }
  ...
  private _screenRayDirection(clientX: number, clientY: number): THREE.Vector3 {
    return this._screenRay(clientX, clientY).direction.normalize();
  }

  private _startThrust(x: number, y: number) {
    this.fly = null;
    this.thrusting = true;
    ...
    if (reverse < 0) this.travelVelocity.addScaledVector(dir, -reverse);
  }

  private _stopThrust(force = false) {
    if (!this.thrusting && !force) return;
  ...
  private _updateThrust(dt: number) {
    if (this.thrusting) {
      ...
      if (reverse < 0) this.travelVelocity.addScaledVector(this.thrustDirection, -reverse);
  ...
  private _refreshLocalGalaxy() {
    ...
    if (this.localGalaxyId) this.localGalaxyCenter.set(...theme.worldOffset);
  }
  ...
  hasZoomAnchor(): boolean { return this.zoomAnchor !== null; }
  ...
  consumeThrustClick(): boolean {
    if (!this.suppressNextClick) return false;
    this.suppressNextClick = false;
    return true;
  }

  consumeOrbitClick(): boolean {
    if (!this.suppressNextLeftClick) return false;
    this.suppressNextLeftClick = false;
    return true;
  }

  update(dt: number) {
    if (this.fly) { this._updateFly(dt); return; }

    this._refreshLocalGalaxy();
    const now = performance.now();
    if (!REDUCED_MOTION && !this.isDragging && !this.thrusting && this.travelVelocity.length() < 1 && now - this.lastUserActivity > IDLE_TRIGGER_MS) this.isIdleDrifting = true;
  ...
  returnToPrevious(_opts: FlyToOptions = {}): boolean {
...
function easeInOutCubic(t: number): number { return t < .5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2; }
