...
export class ThruTheFireSystem {
  group; planetMesh; emberParticles; children = []; labelContainer; time = 0; objectData; clickTargets = [];
  ...
  buildPlanet() {
    ...
    const mat = new THREE.ShaderMaterial({ uniforms: { time: { value: 0 }, crustColor: { value: new THREE.Color(0x180806) }, moltenColor: { value: new THREE.Color(0xe45b28) }, emberGlow: { value: new THREE.Color(0xffaa3d) } }, vertexShader: `varying vec3 vNormal; varying vec3 vPos; uniform float time; void main(){ vNormal=normalize(normalMatrix*normal); vPos=position; vec3 displaced = position + normal * (12.0 * sin(position.y * 0.008 + time * 1.2) * cos(position.z * 0.007 + time * 0.8)); gl_Position = projectionMatrix * modelViewMatrix * vec4(displaced,1.0); }`, fragmentShader: `uniform vec3 crustColor; uniform vec3 moltenColor; uniform vec3 emberGlow; uniform float time; varying vec3 vNormal; varying vec3 vPos; void main(){ vec3 viewDir = normalize(cameraPosition - vPos); float rim = pow(1.0 - max(0.0, dot(vNormal, viewDir)), 2.2); float heat = 0.5 + 0.5 * sin(vPos.x * 0.014 + vPos.y * 0.009 + time * 1.1); vec3 base = mix(crustColor, moltenColor, heat); vec3 finalColor = mix(base, emberGlow, rim * 0.72); gl_FragColor = vec4(finalColor, 1.0); }` });
    ...
    this.group.add(new THREE.PointLight(0xe45b28, 1.45, 7200));
  }
  buildEmbers() {
    const count = 420;
    ...
    this.emberParticles = new THREE.Points(geo, new THREE.PointsMaterial({ color: 0xff8c3a, size: 12, transparent: true, opacity: 0.46, blending: THREE.AdditiveBlending, depthWrite: false }));
    this.group.add(this.emberParticles);
  }
  buildRecordOrbits() {
    [980, 1240, 1520, 1820, 2160, 2520, 2920, 3340].forEach((r) => {
      const ring = new THREE.Mesh(new THREE.RingGeometry(r - 2.2, r + 2.2, 160), new THREE.MeshBasicMaterial({ color: 0xf5f8ff, transparent: true, opacity: 0.1, side: THREE.DoubleSide, depthWrite: false }));
  ...
  buildChildren() {
    ...
    children.forEach((child, i) => {
      ...
      this.children.push({ id: child.id, title: child.title, mediaKind: mk, contentStatus: child.contentStatus ?? 'live', mesh: dec.group, orbitRadius: radii[i], orbitSpeed: speeds[i], orbitAngle: angle, orbitY: heights[i], bobPhase: Math.random() * Math.PI * 2, labelEl: el });
    });
  }
  update(dt, camera, renderer) {
    ...
    for (const c of this.children) {
      ...
      c.mesh.position.set(Math.cos(c.orbitAngle) * c.orbitRadius, c.orbitY + Math.sin(this.time * 0.8 + c.bobPhase) * 22, Math.sin(c.orbitAngle) * c.orbitRadius);
  ...
  updateLabels(camera, renderer) {
    ...
    for (const c of this.children) {
      ...
      const opacity = 1 - Math.min(1, Math.max(0, (dist - 1000) / 4200));
