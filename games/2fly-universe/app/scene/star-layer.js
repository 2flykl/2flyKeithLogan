...
export class StarLayer {
  ...
  _buildFarInstanced() {
    // A tiny faceted star point — no target circles or crosshair geometry.
  ...
  _variantTexture(index) {
    const normalized = ((index % STAR_VARIANTS) + STAR_VARIANTS) % STAR_VARIANTS;
    if (this.atlasVariants.has(normalized)) return this.atlasVariants.get(normalized);
  ...
  _rebuildFar() {
    ...
    for (const s of this.stars) {
      ...
      const hash = this._hashStarId(s.id);
  ...
  update(cameraPos, camera, renderer) {
  ...
  _ensureNearMesh(s) {
    ...
    const hash = this._hashStarId(s.id);
    const texture = this._variantTexture(hash);
  ...
  _updateLabel(s, worldPos, camera, width, height, dist) {
    const LABEL_NEAR = 900;
    const LABEL_FAR = 3000;
  ...
  addStar(star) { this.stars.push(star); this._rebuildFar(); }
