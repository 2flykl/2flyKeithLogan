// Audio Manager — V23 regional spatial crossfade engine.
// Storage access is optional so embedded/modal launches cannot crash during import.
const FADE_DURATION = 1500;

function readMuted() {
  try { return !!window.localStorage?.getItem('universe_muted'); }
  catch { return false; }
}

function writeMuted(value) {
  try {
    if (value) window.localStorage?.setItem('universe_muted', '1');
    else window.localStorage?.removeItem('universe_muted');
  } catch {}
}

class AudioManager {
  ambientLayers = new Map();
  activeRegionTheme = null;
  masterMuted = readMuted();
  masterVol = 0.22;
  _rafId = 0;
  isDucked = false;
  REGION_TRACKS = {
  ...
  unlock() {
    ...
    for (const layer of this.ambientLayers.values()) {
      if (layer.el.paused && layer.targetVol > 0) layer.el.play().catch(() => {});
    }
  }

  setRegionTheme(themeKey) {
    ...
    for (const [src, layer] of this.ambientLayers) {
      if (src !== targetTrack) layer.targetVol = 0;
    }
  ...
  restoreAmbient() {
    ...
    for (const layer of this.ambientLayers.values()) {
      ...
      if (isTarget && layer.el.paused) layer.el.play().catch(() => {});
    }
  }
  ...
  dispose() {
    ...
    for (const layer of this.ambientLayers.values()) layer.el.pause();
