...
import { initUniverseShell } from './app-v23/universe-shell.js?v=23.4.0';
...
async function launch() {
  ...
  if (status) {
    ...
    status.textContent = 'Building galaxies, atlas & depth focus…';
  }
  try {
    ...
    console.error('[2Fly V23.4 launcher]', err);
  }
}
launchBtn?.addEventListener('click', launch);
retryBtn?.addEventListener('click', launch);
window.addEventListener('error', e => console.error('[2Fly V23.4 window error]', e.error || e.message));
window.addEventListener('unhandledrejection', e => console.error('[2Fly V23.4 rejection]', e.reason));
