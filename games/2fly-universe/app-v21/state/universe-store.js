...
// Storage must never block Universe startup. Embedded/sandboxed contexts may deny access.
...
function safeStorageRemove(key) { try { window.localStorage?.removeItem(key); return true; } catch { return false; } }
...
export const store = {
    ...
    toggleMute() {
        ...
        if (next)
            safeStorageSet('universe_muted', '1');
        else
            safeStorageRemove('universe_muted');
        this.set('muted', next);
    },
