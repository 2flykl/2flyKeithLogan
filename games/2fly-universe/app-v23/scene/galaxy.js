...
export class GalaxyScene {
    ...
    updateLabels(camera, renderer, cameraWorldPos, activeGalaxyId = null) {
        ...
        for (const { el, pos, kind } of this.labelEls) {
            ...
            if (activeGalaxyId && activeGalaxyId !== this.data.id)
                opacity *= kind === 'galaxy' ? 0.18 : 0.03;
            if (activeGalaxyId === this.data.id && kind === 'region')
                opacity *= 0.88;
    ...
    update(time, cameraWorldPos) {
        ...
        this.orbitRings.forEach((ring) => {
            ring.material.opacity = (inside ? 0.05 : 0.11) + 0.022 * Math.sin(time * 0.45);
        });
        ...
        if (this.atmospherePoints)
            this.atmospherePoints.rotation.y += 0.00012;
