...
export class GalaxyScene{
  ...
  buildSpiralGPU(theme){
    ...
    if(showcase){
      ...
      const palettes=[
        new THREE.Color(0x48e0d0), // teal
        new THREE.Color(0x67cfff), // cyan-blue
        new THREE.Color(0xf7f3dc), // warm white
        new THREE.Color(0xffc777), // gold
        new THREE.Color(0xb991ff)  // violet accent
      ];
      const layers=[
      ...
      for(let layerIndex=0;layerIndex<layers.length;layerIndex++){
        ...
        for(let i=0;i<count;i++){
          const arm=i%arms;
          // Bias inward just enough for a coherent galaxy while keeping open black gaps between arms.
          ...
          const a=spiral+lane;
          // XY plane faces the default universe camera; Z supplies actual 3D depth instead of a flat image.
          ...
          // Slightly warmer inner galaxy, cooler outer arms.
        ...
        // Slight cinematic tilt; still clearly readable as a spiral from Deep Space.
        pts.rotation.x=-.17;pts.rotation.z=-.08;
        this.group.add(pts);this.gasLayers.push(pts);this.gasMaterials.push(mat);
      }
      return;
    }
    // Preserve the original lighter-weight particle treatment for non-showcase eras.
    ...
    layerScales.forEach((scale,layerIndex)=>{
      ...
      const geo=new THREE.BufferGeometry();geo.setAttribute('aRadius',new THREE.BufferAttribute(radius,1));geo.setAttribute('aTheta',new THREE.BufferAttribute(theta,1));geo.setAttribute('aHeight',new THREE.BufferAttribute(height,1));geo.setAttribute('aSeed',new THREE.BufferAttribute(seed,1));geo.setAttribute('size',new THREE.BufferAttribute(sizes,1));geo.setAttribute('aColor',new THREE.BufferAttribute(colors,3));
      const mat=new THREE.ShaderMaterial({uniforms:{time:{value:0},globalAlpha:{value:baseOpacity[layerIndex]},twist:{value:.00108},maxRadius:{value:maxRadius}},vertexShader:`attribute float aRadius;attribute float aTheta;attribute float aHeight;attribute float aSeed;attribute float size;attribute vec3 aColor;uniform float time;uniform float globalAlpha;uniform float twist;uniform float maxRadius;varying vec3 vColor;varying float vAlpha;void main(){float differential=(1.30-0.50*(aRadius/maxRadius));float a=aTheta+aRadius*twist+time*.0042*differential;float wobble=sin(time*.13+aSeed*21.0+aRadius*.003)*8.0;vec3 p=vec3(cos(a)*aRadius,aHeight+wobble,sin(a)*aRadius);vColor=aColor;float flick=.84+.16*sin(time*(.55+aSeed*.55)+aSeed*37.0);vAlpha=globalAlpha*flick;vec4 mv=modelViewMatrix*vec4(p,1.0);gl_Position=projectionMatrix*mv;gl_PointSize=clamp(size*(650.0/-mv.z),.55,9.0);}`,fragmentShader:`varying vec3 vColor;varying float vAlpha;void main(){vec2 uv=gl_PointCoord-.5;float d=length(uv);if(d>.5)discard;float soft=smoothstep(.5,.03,d);float core=smoothstep(.13,0.0,d);gl_FragColor=vec4(vColor*(.78+core*1.35),soft*vAlpha);}`,transparent:true,depthWrite:false,blending:THREE.AdditiveBlending});
  ...
  buildProceduralNebula(theme){
    ...
    const layerCount=showcase?3:2;
    for(let i=0;i<layerCount;i++){
      const geo=new THREE.SphereGeometry(this.atmosphereRadius*(showcase?(0.70+i*0.12):(0.72+i*.09)),48,32);
      const densityVal=showcase?(0.040*(1-i*0.16)):(0.05*(1-i*.18));
      const mat=new THREE.ShaderMaterial({uniforms:{time:{value:0},colorA:{value:new THREE.Color(theme.primaryColor)},colorB:{value:new THREE.Color(theme.accentColor)},density:{value:densityVal}},vertexShader:vertex,fragmentShader:fragment,transparent:true,depthWrite:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending});
      const mesh=new THREE.Mesh(geo,mat);mesh.scale.y=showcase?(0.24+i*0.03):(0.20+i*.025);mesh.rotation.set(.08+i*.025,i*.7,.05-i*.02);this.group.add(mesh);this.nebulaLayers.push(mesh);this.nebulaMaterials.push(mat);
    }
  }
  buildCore(theme){
    const showcase=theme.status==='showcase';
    if(showcase){
      // V5: NO core sphere, NO green halo. A galactic nucleus is a dense stellar population + thin warm disk.
      ...
      const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.BufferAttribute(pos,3));geo.setAttribute('size',new THREE.BufferAttribute(sizes,1));geo.setAttribute('aColor',new THREE.BufferAttribute(colors,3));geo.setAttribute('aSeed',new THREE.BufferAttribute(seed,1));
      const mat=new THREE.ShaderMaterial({uniforms:{time:{value:0}},vertexShader:`attribute float size;attribute vec3 aColor;attribute float aSeed;uniform float time;varying vec3 vColor;varying float vAlpha;void main(){vec3 p=position;float a=time*.012;float cs=cos(a),sn=sin(a);p.xy=mat2(cs,-sn,sn,cs)*p.xy;vColor=aColor;vAlpha=.72+.28*sin(time*(.7+aSeed)+aSeed*39.0);vec4 mv=modelViewMatrix*vec4(p,1.0);gl_Position=projectionMatrix*mv;gl_PointSize=clamp(size*(800.0/max(1.0,-mv.z)),.8,10.0);}`,fragmentShader:`varying vec3 vColor;varying float vAlpha;void main(){vec2 q=gl_PointCoord-.5;float d=length(q);if(d>.5)discard;float a=smoothstep(.5,.04,d)*vAlpha;gl_FragColor=vec4(vColor*(1.0+smoothstep(.18,0.0,d)),a);}`,transparent:true,depthWrite:false,blending:THREE.AdditiveBlending});
      ...
      // Thin luminous nucleus rings — geometry, not a filled glowing object.
      this.nucleusRings=[];
      for(let i=0;i<3;i++){
      ...
      // Warm-white light source for planet shading. Invisible light, no visible ball.
  ...
  buildAbstractWhiteLines(theme){
    // Thin white galaxy-only grooves: a vinyl-record navigation field.
