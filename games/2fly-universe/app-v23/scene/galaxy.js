...
export class GalaxyScene{
  ...
  buildSpiralGPU(theme){
    ...
    if(showcase){
      ...
      const layers=[
        {count:26000,opacity:.42,sizeMin:12.0,sizeMax:38.0,width:.58,scatter:260,depth:620},
        {count:9000, opacity:.64,sizeMin:5.0,sizeMax:18.0,width:.25,scatter:120, depth:340},
        {count:2600, opacity:.86,sizeMin:9.0,sizeMax:27.0,width:.065,scatter:38, depth:75}
      ];
      for(let layerIndex=0;layerIndex<layers.length;layerIndex++){
        const L=layers[layerIndex],count=Math.floor(L.count*(matchMedia('(max-width:700px)').matches?.5:1));
  ...
  buildProceduralNebula(theme){
    ...
    if(showcase){
      // Exterior LOD provides the readable spiral. Inside, keep space clear: no giant nebula plate.
      return;
    }
  ...
  update(time,cameraWorldPos){
    ...
    this.thresholdState=inside; // Visual LOD only; navigation boundaries have one owner.
    ...
    this.gasMaterials.forEach((m,i)=>{m.uniforms.time.value=time;const mult=GALAXY_THEMES[this.data.id]?.status==='uncharted'?.55:1;if(isShowcase){const approach=THREE.MathUtils.clamp((worldRadius*2.15-dist)/(worldRadius*1.1),0,1);m.uniforms.globalAlpha.value=base[i]*(.88+.22*approach)*(inside?.88:1);}else{m.uniforms.globalAlpha.value=base[i]*(inside?.34:1)*mult;}});
    this.nebulaMaterials.forEach((m,i)=>{m.uniforms.time.value=time;});if(this.stellarNucleusMat)this.stellarNucleusMat.uniforms.time.value=time;this.nucleusRings?.forEach((r,i)=>{r.rotation.z+=motion*.00006*(i%2?1:-1);r.material.opacity=.12+.055*(.5+.5*Math.sin(time*.42+i));});if(this.coreDisk?.material?.uniforms?.time)this.coreDisk.material.uniforms.time.value=time;this.coreRings?.forEach((r,i)=>{r.rotation.z=time*(i%2?-.035:.028)+i*.41;r.material.opacity=(isShowcase?.26:.18)+.05*Math.sin(time*.7+i);});
    ...
    this.abstractLines.forEach((e,i)=>{const breathe=.76+.24*Math.sin(time*(.16+(i%3)*.025)+e.phase);e.line.material.opacity=e.baseOpacity*(inside?1.22:1)*breathe;e.line.rotation.y+=motion*.000018*(i%2?1:-1);});
    ...
    this.nebulaLayers.forEach((n,i)=>{n.rotation.z+=motion*.000018*(i%2?1:-1);});
    this.ledPivots.forEach(l=>{l.pivot.rotation.y+=motion*l.speed*.003;l.pivot.rotation.x+=motion*l.speed*.001;l.node.material.opacity=(inside?.28:.48)+.09*Math.sin(time*.8+l.speed*20);});
