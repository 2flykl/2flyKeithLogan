...
export class InterstellarCurrents {
    ...
    constructor() {
        ...
        for (let i = 0; i < ids.length - 1; i++)
            this.buildCurrent(ids[i], ids[i + 1], i);
    ...
    buildCurrent(aId, bId, seed, opacity = 0.22) {
    ...
    update(dt, time) {
        for (const p of this.particles) {
            ...
            p.mesh.scale.setScalar(0.72 + 0.35 * (0.5 + 0.5 * Math.sin(time * 2 + p.t * 18)));
        }
        for (let i = 0; i < this.ribbons.length; i++)
            this.ribbons[i].material.opacity = 0.13 + 0.07 * (0.5 + 0.5 * Math.sin(time * 0.35 + i));
    }
}
