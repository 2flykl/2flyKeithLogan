...
import { createOverlayEl, injectOverlayStyles, trapFocus } from './overlay-utils.js';
import { starRepository } from '../data/star-repository.js';
import { store } from '../state/universe-store.js';
import { openStarCardOverlay } from './star-card-export.js';
export function openStarPlacementOverlay(container, ctx, onClose) {
    injectOverlayStyles();
    ...
    function requestExit() {
        ...
        if (step === 'info') {
            const dnInput = panel.querySelector('#place-display-name');
            const snInput = panel.querySelector('#place-star-name');
            const msgInput = panel.querySelector('#place-message');
            if (dnInput)
                displayName = dnInput.value.trim();
            if (snInput)
                starName = snInput.value.trim();
            if (msgInput)
    ...
    function bindStepEvents() {
        ...
        if (step === 'info') {
            const nextBtn = panel.querySelector('#place-next');
            nextBtn?.addEventListener('click', () => {
                const dn = (panel.querySelector('#place-display-name')?.value ?? '').trim();
                const sn = (panel.querySelector('#place-star-name')?.value ?? '').trim();
                const msg = (panel.querySelector('#place-message')?.value ?? '').trim();
                if (!dn) {
        ...
        if (step === 'confirm') {
            ...
            panel.querySelector('#place-confirm')?.addEventListener('click', async () => {
                const btn = panel.querySelector('#place-confirm');
                if (btn) {
                    btn.disabled = true;
                ...
                if (result.success && result.star) {
                    ...
                    setTimeout(() => {
                        if (result.star)
                            openStarCardOverlay(container, result.star, () => close(true));
                    }, 2200);
                }
                else {
                    const errMap = {
                    ...
                    const err = panel.querySelector('#place-error');
                    if (err)
                        err.textContent = errMap[result.error ?? 'server-error'] ?? 'An error occurred.';
                }
            });
        }
    }
...
function stepHTML(step, ctx, dn, sn, msg, showUnsavedWarning) {
