...
function createProceduralPlanetTexture(objectId, accentColorHex) {
    ...
    // Material-specific details are painted into a genuine equirectangular wrap.
    c.save();
...
export function createDecoratedPlanet(objectId, size, accentColorHex) {
    // V5.1 MATERIAL LOCK: preserve the exact V5 world positions and universe scale.
    // Only the visual representation changes: sprite -> true UV-mapped 3D sphere.
    const group = new THREE.Group();
    const radius = size * 1.05; // matches the old sprite's visible footprint without scaling the universe.
    ...
    // Atmosphere / material rim: transparent shell, still real 3D geometry.
    ...
    // Thin celestial ring kept at the same local footprint as V5.
