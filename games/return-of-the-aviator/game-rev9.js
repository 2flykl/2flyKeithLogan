...
loadAll().then(m=>{ const b=document.getElementById('start'); b.disabled=false; b.textContent=m?`START EXPERIENCE (${m} OPTIONAL ASSETS MISSING)`:'START EXPERIENCE'; const isIframe = window.self !== window.top; const isAuto = new URLSearchParams(location.search).get('autostart')==='1'; if(isIframe || isAuto) start(); }).catch(e=>{ fatal.style.display='block'; fatal.textContent='LOAD ERROR: '+e.message; });
