...
class Game{
 ...
 reset(){this.scene='intro';this.sceneTime=0;this.totalTime=0;this.score=0;this.combo=0;this.maxCombo=0;this.hits=0;this.shots=0;this.shake=0;this.flash=0;this.transition=0;this.message='';this.messageT=0;this.altitude=18500;this.weapon='WHOLE NOTE';this.weaponSecured=false;this.braced=false;this.director=new Director();this.player={x:W*.5,y:H*.32,vx:0,vy:0,jumpY:0,jumping:false,hp:5,inv:0,tilt:0,flow:0,special:1};this.enemies=[];this.bullets=[];this.enemyBullets=[];this.particles=[];this.obstacles=[];this.roadTurn=0;this.roadTurnTarget=0;this.roadScroll=0;this.runwayReverse=false;this.bossArenaPhase=0;this.floorBreak=0;this.stars=[];for(let i=0;i<160;i++)this.stars.push({x:rnd(0,W),y:rnd(0,H),z:rnd(.2,1)});}
 ...
 startScene(s){
   this.scene=s;this.sceneTime=0;this.player.inv=1.1;this.director.spawnClock=.6;this.player.jumpY=0;this.player.jumping=false;this.player.vy=0;
   if(s==='skydive'){this.player.x=W*.5;this.player.y=H*.28;this.altitude=18500;this.message='FREEFALL CONTROL ACTIVE';this.messageT=2.2}
   else if(s==='runway'){this.player.x=W*.5;this.player.y=H*.77;this.runwayReverse=false;this.message='RUNWAY ESCAPE';this.messageT=2.0}
   else if(s==='car'){this.player.x=W*.5;this.player.y=H*.78;this.roadTurn=0;this.roadTurnTarget=0;this.roadScroll=0;this.message='ALGORITHM MAZE';this.messageT=2.0}
   else if(s==='boss'){this.message='ALGORITHM AIR BOSS';this.messageT=2.2;this.bossArenaPhase=0;this.floorBreak=0;this.boss={x:W*.5,y:150,hp:120,max:120,t:0,phase:0,hitFlash:0,scale:1}}
 }
 ...
 updatePlayer(dt){
   let dir=(keys.right?1:0)-(keys.left?1:0),max=this.scene==='skydive'?420:this.scene==='car'?520:430,accel=this.scene==='car'?1600:1350;
   this.player.vx+=dir*accel*dt;this.player.vx*=Math.pow(.0008,dt);this.player.vx=clamp(this.player.vx,-max,max);this.player.x+=this.player.vx*dt;this.player.x=clamp(this.player.x,110,W-110);
   this.player.tilt=lerp(this.player.tilt,dir*.28+this.player.vx/max*.14,dt*6);this.player.flow=clamp(this.player.flow+dt*(Math.abs(dir)>.2?.008:.018),0,1);
   if((this.scene==='runway'||this.scene==='boss')&&keys.up&&!this.player.jumping){this.player.jumping=true;this.player.vy=-720;keys.up=false;audio.sfx(380,.08,'square',.07)}
   if((this.scene==='runway'||this.scene==='boss')&&this.player.jumping){this.player.vy+=1650*dt;this.player.jumpY+=this.player.vy*dt;if(this.player.jumpY>=0){this.player.jumpY=0;this.player.vy=0;this.player.jumping=false}}
 ...
 spawnSkyEnemy(){let type=this.director.enemyType(),x=rnd(90,W-90),speed=rnd(180,300)*(1+this.director.pressure*.45);this.enemies.push({type,x,y:H+60,vx:rnd(-35,35),vy:-speed,hp:type==='shield'?3:1,r:type==='swarm'?18:26,t:0,fire:rnd(.5,1.4),hitFlash:0,hitKick:0})}
 updateRunway(dt){
   this.fireCd=Math.max(0,(this.fireCd||0)-dt);const c=audio.cues||{diveEnd:40,runwayEnd:69};const p=audio.progress(c.diveEnd,c.runwayEnd,this.sceneTime+c.diveEnd);
   if(p>.50&&!this.runwayReverse){this.runwayReverse=true;this.message='TURN! RUN TOWARD CAMERA!';this.messageT=2.0;this.shake=7}
   this.director.spawnClock-=dt;if(this.director.spawnClock<=0){this.director.spawnClock=this.director.interval()*(.98-.22*p);this.spawnRunwayHazard()}
   if(p>=.995)this.cut('car','VEHICLE PURSUIT');this.updateEnemies(dt,'runway');for(const o of this.obstacles){o.z-=dt*o.speed;o.y=lerp(210,H+90,1-o.z);o.x=lerp(W*.5,(o.lane-.5)*360+W*.5,1-o.z);o.scale=lerp(.15,1.5,1-o.z)}this.obstacles=this.obstacles.filter(o=>o.z>-0.1)
 }
 ...
 updateCar(dt){
   this.fireCd=Math.max(0,(this.fireCd||0)-dt);const c=audio.cues||{runwayEnd:69,carEnd:98};const p=audio.progress(c.runwayEnd,c.carEnd,this.sceneTime+c.runwayEnd);this.roadScroll=(this.roadScroll+dt*(.55+.55*p))%1;
   if(Math.abs(this.roadTurn-this.roadTurnTarget)<.04&&Math.random()<dt*(.28+.5*p))this.roadTurnTarget=rnd(-.72,.72);this.roadTurn=lerp(this.roadTurn,this.roadTurnTarget,dt*(.7+1.2*p));this.director.spawnClock-=dt;if(this.director.spawnClock<=0){this.director.spawnClock=this.director.interval()*(.92-.20*p);this.spawnCarHazard()}
   if(p>=.995)this.cut('boss','FINAL APPROACH');this.updateEnemies(dt,'car');for(const o of this.obstacles){o.z-=dt*o.speed;let depth=1-o.z,curve=Math.sin((depth*2.8+this.roadScroll)*Math.PI)*this.roadTurn*330*depth;o.y=lerp(235,H+110,depth);o.x=lerp(W*.5,(o.lane-.5)*390+W*.5,depth)+curve;o.scale=lerp(.12,1.6,depth)}this.obstacles=this.obstacles.filter(o=>o.z>-0.1)
 }
 ...
 updateBoss(dt){
   this.fireCd=Math.max(0,(this.fireCd||0)-dt);const c=audio.cues||{carEnd:98,bossPeak:118,end:126};const p=audio.progress(c.carEnd,c.end,this.sceneTime+c.carEnd);let b=this.boss;b.t+=dt;b.hitFlash=Math.max(0,(b.hitFlash||0)-dt*4);const lost=1-b.hp/b.max;
   if(lost>=.33&&this.bossArenaPhase===0){this.bossArenaPhase=1;this.floorBreak=1.8;this.message='GROUND FAILURE!';this.messageT=2.0;this.shake=24;this.flash=.45}
   if(this.floorBreak>0){this.floorBreak-=dt;if(this.floorBreak<=0){this.bossArenaPhase=2;this.player.inv=1.5;this.message='LOWER GRID — KEEP FIRING';this.messageT=2.3}}
   b.scale=clamp(.9+.23*Math.sin(b.t*.55)+.18*Math.sin(b.t*.17)+(lost>.55?.18:0),.68,1.35);b.x=W*.5+Math.sin(b.t*(.70+.35*p))*260;b.y=145+Math.sin(b.t*1.23)*35;const volley=lerp(1.35,.72,p);
   if((b.t%volley)<dt){let ang=Math.atan2(this.player.y+this.player.jumpY-b.y,this.player.x-b.x);for(let k=-1;k<=1;k++)this.enemyBullets.push({x:b.x,y:b.y,vx:Math.cos(ang+k*.14)*330,vy:Math.sin(ang+k*.14)*330,r:8,life:4})}
   if((b.t%lerp(5.4,3.5,p))<dt){this.message='LASER SWEEP — MOVE / JUMP';this.messageT=1.1;for(let k=0;k<9;k++)this.enemyBullets.push({x:b.x,y:b.y,vx:(k-4)*65,vy:390,r:7,life:3.2})}if(p>.82&&b.hp>14)b.hp-=dt*2.6;if(b.hp<=0||p>=.995){this.spawnExplosion(b.x,b.y,2.4);this.score+=10000;this.scene='ending';this.sceneTime=0;this.message='ALGORITHM CORE BROKEN';this.messageT=4}
 }
 ...
 collisions(){for(const b of this.bullets){for(const e of this.enemies){let dx=b.x-e.x,dy=b.y-e.y;if(b.life>0&&e.hp>0&&dx*dx+dy*dy<(e.r+9)**2){b.life=0;e.hp-=b.pow;e.hitFlash=.45;e.hitKick=(b.vx>=0?1:-1)*120;this.hits++;this.combo++;this.maxCombo=Math.max(this.maxCombo,this.combo);this.score+=100*this.combo;this.spawnExplosion(e.x,e.y,e.hp<=0?.6:.18);if(e.hp<=0)audio.sfx(120,.13,'sawtooth',.08)}}if(this.scene==='boss'){let bo=this.boss,dx=b.x-bo.x,dy=b.y-bo.y;if(b.life>0&&dx*dx+dy*dy<125**2){b.life=0;bo.hp-=b.pow;bo.hitFlash=.35;this.hits++;this.combo++;this.score+=80*this.combo;this.shake=2}}}
 ...
 for(const o of this.obstacles){if(!o.hit&&o.z<.11){let laneX=(o.lane-.5)*360+W*.5;if(Math.abs(this.player.x-laneX)<92 && !(this.scene==='runway'&&this.player.jumpY<-65)){o.hit=true;this.damage()}else{this.score+=250;this.combo++;this.player.flow=clamp(this.player.flow+.13,0,1);audio.sfx(520,.04,'sine',.04)}}}}
 ...
 drawSkydive(){let zoom=this.altitude>10000?1.08:this.altitude>4000?.92:.78;ctx.save();ctx.translate(W/2,H/2);ctx.scale(zoom,zoom);ctx.translate(-W/2,-H/2);this.bgGradient('#163b63','#06111f');for(const s of this.stars){ctx.fillStyle=`rgba(180,225,255,${.18+s.z*.45})`;ctx.fillRect(s.x,s.y,1+s.z*2,10+s.z*35)}for(let i=0;i<8;i++){let y=((i*170+this.totalTime*210)%950)-120;ctx.fillStyle='rgba(230,245,255,.08)';ctx.beginPath();ctx.ellipse((i*237)%W,y,200,45,0,0,TAU);ctx.fill()}for(const e of this.enemies)this.drawBot(e.x,e.y,e.r,e);for(const b of this.bullets)this.drawNote(b.x,b.y,b.r,'#5ee6ff',true);for(const b of this.enemyBullets)this.drawOrb(b.x,b.y,b.r,'#ff493f');this.drawDiver(this.player.x,this.player.y);if(this.weaponSecured&&this.altitude>500){ctx.strokeStyle='rgba(80,220,255,.35)';ctx.lineWidth=4;ctx.beginPath();ctx.arc(this.player.x,this.player.y,70+Math.sin(this.totalTime*8)*8,0,TAU);ctx.stroke()}ctx.restore();if(this.altitude<2600&&!this.weaponSecured){ctx.fillStyle='rgba(78,211,255,.85)';ctx.font='700 20px Arial';ctx.textAlign='center';ctx.fillText('W.M.P. SIGNAL ↓',W/2,H-80)}}
 drawRunway(){
   this.bgGradient('#0f1c28','#030508');this.drawPerspectiveRoad('#27313b','#e6edf5');for(let i=0;i<8;i++){let z=((i/8+this.totalTime*.17)%1),y=lerp(225,H,z),xspan=lerp(40,600,z);ctx.fillStyle='rgba(255,50,45,.8)';ctx.fillRect(W/2-xspan,y,6+z*5,4);ctx.fillRect(W/2+xspan,y,6+z*5,4)}
   const c=audio.cues||{diveEnd:40,runwayEnd:69};const p=audio.progress(c.diveEnd,c.runwayEnd,this.sceneTime+c.diveEnd);if(this.runwayReverse){let q=clamp((p-.5)/.5,0,1);ctx.save();ctx.translate(W*.5,lerp(150,260,q));ctx.scale(lerp(.22,1.35,ease(q)),lerp(.22,1.35,ease(q)));this.drawAlgorithmBoss(0,0,1);ctx.restore()}
   for(const o of this.obstacles)this.drawObstacle(o);for(const e of this.enemies)this.drawBot(e.x,e.y,e.r,e);for(const b of this.bullets)this.drawNote(b.x,b.y,b.r,'#5ee6ff',false);for(const b of this.enemyBullets)this.drawOrb(b.x,b.y,b.r,'#ff493f');this.drawRunner(this.player.x,this.player.y,this.runwayReverse)
 }
 drawRunner(x,y,front=false){
   ctx.save();ctx.translate(x,y+this.player.jumpY);ctx.rotate(this.player.tilt*.15);ctx.fillStyle='#111720';ctx.fillRect(-26,-28,52,76);ctx.fillStyle='#eef3f6';ctx.fillRect(-31,-70,62,56);ctx.fillStyle='#8d5a3d';ctx.beginPath();ctx.arc(0,-85,17,0,TAU);ctx.fill();ctx.fillStyle='#05080b';ctx.fillRect(-17,-91,34,6);
   let phase=Math.sin(this.totalTime*15)*24;ctx.strokeStyle='#151a21';ctx.lineWidth=17;ctx.beginPath();ctx.moveTo(-10,42);ctx.lineTo(-23+phase,88);ctx.moveTo(10,42);ctx.lineTo(23-phase,88);ctx.stroke();ctx.fillStyle='#121a24';ctx.fillRect(front?-38:18,-58,46,14);ctx.fillStyle='#59dfff';ctx.fillRect(front?-42:57,-55,8,8);ctx.restore()
 }
 ...
 drawBot(x,y,r,e=null){
   ctx.save();ctx.translate(x+(e?.hitKick||0)*.04,y);ctx.fillStyle=(e&&e.hitFlash>0)?'#f3f7ff':'#171c24';ctx.strokeStyle=(e&&e.hitFlash>0)?'#8fe8ff':'#596473';ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,r,0,TAU);ctx.fill();ctx.stroke();ctx.strokeStyle='#3b4651';
   for(let i=0;i<6;i++){let a=i/6*TAU;ctx.beginPath();ctx.moveTo(Math.cos(a)*r*.6,Math.sin(a)*r*.6);ctx.lineTo(Math.cos(a)*r*1.55,Math.sin(a)*r*1.55);ctx.stroke()}
   ctx.fillStyle=e&&e.hitFlash>0?'#ffffff':'#ff3a32';ctx.shadowColor=e&&e.hitFlash>0?'#8fe8ff':'#ff3a32';ctx.shadowBlur=16;ctx.beginPath();ctx.arc(0,0,r*.35,0,TAU);ctx.fill();ctx.shadowBlur=0;ctx.restore()
 }
...
window.addEventListener('keydown',e=>{if(['ArrowLeft','ArrowRight','ArrowUp','Space'].includes(e.code))e.preventDefault();if(e.code==='ArrowLeft'||e.code==='KeyA')keys.left=true;if(e.code==='ArrowRight'||e.code==='KeyD')keys.right=true;if(e.code==='ArrowUp'||e.code==='KeyW')keys.up=true;if(e.code==='Space')keys.fire=true;if(e.code==='ShiftLeft'||e.code==='ShiftRight')keys.special=true;if(e.code==='KeyP'){paused=!paused;paused?audio.pause():audio.resume()}});
window.addEventListener('keyup',e=>{if(e.code==='ArrowLeft'||e.code==='KeyA')keys.left=false;if(e.code==='ArrowRight'||e.code==='KeyD')keys.right=false;if(e.code==='ArrowUp'||e.code==='KeyW')keys.up=false});
document.querySelectorAll('#touch button').forEach(b=>{let k=b.dataset.key;const down=e=>{e.preventDefault();if(k==='left'||k==='right'||k==='up')keys[k]=true;else if(k==='fire')keys.fire=true};const up=e=>{e.preventDefault();if(k==='left'||k==='right'||k==='up')keys[k]=false};b.addEventListener('pointerdown',down);b.addEventListener('pointerup',up);b.addEventListener('pointercancel',up)});
})();