...
export class TourBuilder {
  ...
  private render() {
    ...
    this.panel.querySelector('#tour-launch')?.addEventListener('click',()=>{ if(this.stops.length){ this.close(); this.callbacks.onPlay([...this.stops]); }});
    this.panel.querySelectorAll<HTMLElement>('[data-add]').forEach(el=>el.addEventListener('click',()=>{ const s=this.available.find(x=>x.id===el.dataset.add); if(s)this.add(s); }));
