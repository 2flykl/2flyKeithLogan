...
import { createOverlayEl, injectOverlayStyles, closeOnEsc, addCloseButton, trapFocus } from './overlay-utils';
import { store } from '../state/universe-store';

export function openStarCardOverlay(
  ...
  if (wrap) wrap.appendChild(previewCanvas);
  ...
  panel.querySelector('#copy-link')?.addEventListener('click', async () => {
    try {
      ...
      if (conf) { conf.textContent = 'Link copied!'; setTimeout(() => { conf.textContent = ''; }, 2000); }
    } catch {
      const conf = panel.querySelector('#copy-confirm');
      if (conf) conf.textContent = shareUrl;
    }
  });
...
export function openStarViewOverlay(
  container: HTMLElement,
  star: StarRecord,
  onClose: () => void
) {
...
export async function playStarArrivalSequence(
  container: HTMLElement,
  star: StarRecord,
  onComplete: () => void
) {
...
function dlBtnStyle(bg = 'rgba(20,40,80,0.6)'): string {
  return [
...
function drawStarCard(canvas: HTMLCanvasElement, star: StarRecord, w: number, h: number) {
