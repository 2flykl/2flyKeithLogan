...
export class EraOrbitSystem {
    ...
    constructor(galaxyId) {
        ...
        defs.forEach((def, i) => {
            ...
            switch (def.form) {
                case 'faceted':
                    geo = new THREE.IcosahedronGeometry(def.radius, 2);
                    break;
                case 'torus':
                    geo = new THREE.TorusKnotGeometry(def.radius * 0.72, def.radius * 0.19, 96, 16, 2, 5);
                    break;
                case 'crystal':
                    geo = new THREE.OctahedronGeometry(def.radius, 1);
                    break;
                case 'station':
                    geo = new THREE.CylinderGeometry(def.radius * 0.72, def.radius, def.radius * 1.8, 10, 2);
                    break;
            ...
            pivot.add(line);
            if (def.ring) {
                const torus = new THREE.Mesh(new THREE.TorusGeometry(def.radius * 1.5, 28, 12, 128), new THREE.MeshBasicMaterial({ color: theme.accentColor, transparent: true, opacity: 0.22, depthWrite: false, blending: THREE.AdditiveBlending }));
                torus.rotation.x = Math.PI / 2.35;
                body.add(torus);
                const torus2 = torus.clone();
                torus2.scale.setScalar(1.14);
                torus2.rotation.y = Math.PI / 4;
                torus2.material = torus.material.clone();
                torus2.material.opacity = 0.08;
                body.add(torus2);
            }
            if (def.form === 'station') {
                const antenna = new THREE.Mesh(new THREE.TorusGeometry(def.radius * 0.95, 14, 8, 64), new THREE.MeshBasicMaterial({ color: theme.starTint, transparent: true, opacity: 0.3 }));
