...
export class Photographer {
 ...
 onEvent(type,g){if(type==='charge'){this.shotPhase='';this.play('ready')}if(type==='release'){this.lastAttempt=g.attempts;this.play('raise',g.attempts%2?['shoot']:['crouch_in','crouch'])}if(type==='caught'||type==='miss'){this.targetX=g.attempts%2?1.95:1.4;this.targetZ=g.attempts%2?-5.8:-5.4;this.recover();}if(type==='ready'){this.targetX=g.attempts%2?1.95:1.4;this.targetZ=g.attempts%2?-5.8:-5.4;this.recover();}}
 ...
 step(dt,g,{frozen=false,cutaway=false}={}){
  ...
  if(g.phase==='charging'&&this.state==='idle')this.play('ready');
  if(g.phase==='flight'&&u>.63&&['kneel','kneel_in','crouch','crouch_in'].includes(this.state))this.recover();
