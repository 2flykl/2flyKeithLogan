(() => {
  ...
  function viewPath(roomId, index) {
    return `assets/rooms/${roomId}_${index}.jpg`;
  }
  ...
  function collectPreloadUrls() {
    ...
    DATA.rooms.forEach((room) => {
      ...
      urls.push(`assets/art/${room.id}.jpg`);
  ...
  async function preload() {
    ...
    if (failed) {
      els.loadText.textContent = `${failed} images could not load. Extract the complete ZIP, then reopen index.html.`;
      const retry = document.createElement('button');
      retry.className = 'primary-button'; retry.textContent = 'RETRY LOADING';
      retry.addEventListener('click', () => location.reload());
      els.loadText.after(retry);
      return;
    }
    window.setTimeout(() => {
  ...
  async function startRoom(index) {
    ...
    els.roomTransition.style.backgroundImage = `linear-gradient(#08100de8,#08100ddb),url("assets/art/${room.id}.jpg")`;
    ...
    updateTimer();
    startTimer();
    setFeedback(room.isExitRoom ? "FRONT EXIT OPENS AT HALFWAY · UP WHEN THE SIGNAL APPEARS" : "LEFT / RIGHT TO TURN / TOGGLE · UP TO GRAB", "");
  }
  ...
  function renderView(instant = false, direction = 1) {
    ...
    const dots = document.getElementById('bearingDots');
    dots.innerHTML = DATA.degrees.map((d,i) => `<i class="${i === viewIndex ? 'current' : ''}" title="${d} degrees"></i>`).join('');
    els.roomImage.alt = `${room.name}, ${DATA.directions[viewIndex].toLowerCase()} view`;
    ...
    window.setTimeout(() => {
      if (turnToken !== runToken || turnScenario !== scenario || !playing) return;
      els.roomImage.src = url;
      els.ambientBackdrop.style.backgroundImage = `url("${url}")`;
      els.roomViewport.classList.remove("turning-left", "turning-right");
      registerViewVisit();
  ...
  function grab() {
    ...
    const collectedScenario = scenario;
    const collectedView = viewIndex;
    els.itemAnchor.classList.add("collected");
    els.grabBtn.disabled = true;

    window.setTimeout(() => {
      entry.status = "saved";
      if (scenario !== collectedScenario || !playing) return;
      ...
      const slot = scenario.revealSlots.get(collectedView);
  ...
  function finishRoom(reason = "exit") {
    ...
    window.setTimeout(() => {
      if (scenario !== completedScenario || !playing) return;
      els.itemAnchor.classList.remove("collected", "second-look-reveal");
      els.findExitBanner.hidden = true;
      els.exitSignal.hidden = true;
  ...
  function failRoom() {
    ...
    const failedToken = runToken;
    window.setTimeout(() => {
      if (scenario !== failedScenario || runToken !== failedToken) return;
      finishGame({
        failed: true,
  ...
  function startTimer() {
    ...
    timerId = window.setInterval(() => {
      ...
      if (graceTimeLeft > 0) {
        ...
        document.querySelector(".scene-timer").classList.add("orientation");
        els.timerText.textContent = "READY";
        document.getElementById("timerPhase").textContent = "GET YOUR BEARINGS";
  ...
  function updateTimer() {
    document.querySelector(".scene-timer").classList.remove("orientation");
    const pct = Math.max(0, Math.min(1, timeLeft / totalTime));
    const elapsed = 1 - pct;
    els.roomViewport.style.setProperty('--urgency', elapsed.toFixed(3));
    document.getElementById('timerPhase').textContent = pct < .28 ? 'GET OUT NOW' : scenario?.exitActive ? 'FIND THE EXIT' : 'ESCAPE IN';
    const seconds = Math.ceil(timeLeft);
    if (playing && seconds > 0 && seconds <= 3 && seconds !== lastWarning) { lastWarning = seconds; playSynthCue("focus"); }
  ...
  function finishGame(result = {}) {
    ...
    els.reflectionText.textContent = `${result.failed ? `${roomIndex} of ${DATA.rooms.length} rooms exited.` : `All ${DATA.rooms.length} rooms exited.`} ${reflection}`;
  ...
  async function startAudio() {
    ...
    try {
      ...
      els.audioBtn.textContent = 'SONG ON';
      els.audioBtn.setAttribute('aria-label', 'Mute soundtrack');
    } catch (_) {
      els.audioBtn.textContent = 'PLAY SONG';
      els.audioBtn.setAttribute('aria-label', 'Retry soundtrack playback');
      setFeedback('TAP PLAY SONG TO START THE SOUNDTRACK', 'warn');
    }
  ...
  function pauseAudio() {
    els.music.pause();
    fireAudio?.suspend();
    synthAudioCtx?.suspend();
  }

  function toggleAudio() {
    if (audioEnabled && playing && els.music.paused) { startAudio(); return; }
    audioEnabled = !audioEnabled;
    els.audioBtn.textContent = audioEnabled ? 'SONG ON' : 'SONG OFF';
    els.audioBtn.setAttribute('aria-label', audioEnabled ? 'Mute soundtrack' : 'Enable soundtrack');
  ...
  function exitExperience() {
    ...
    els.roomTransition.classList.add('hidden');
    els.introOverlay.classList.remove('hidden');
    els.game.classList.remove('exit-phase','room-failed','time-critical');
    els.findExitBanner.hidden = true;
    els.exitSignal.hidden = true;
    setBaseScene();
  }
  ...
  // Canvas effects are presentation only; no game state or random item decisions here.
  const canvas = document.getElementById('atmosphere');
  const paint = canvas.getContext('2d');
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  let effectFrame = 0;
  function atmosphere(now) {
    requestAnimationFrame(atmosphere);
    if (++effectFrame % 2 || document.hidden) return;
    const w = canvas.clientWidth, h = canvas.clientHeight;
    if (canvas.width !== w || canvas.height !== h) { canvas.width = w; canvas.height = h; }
    paint.clearRect(0,0,w,h);
    if (!playing || !scenario || reducedMotion) return;
    const progress = Math.max(0,Math.min(1,1-timeLeft/totalTime)), t = now/1000;
    const heat = progress*progress;
    // Smoke descends from the ceiling and curls at the edges, leaving the choice area clear.
    paint.globalCompositeOperation = 'source-over';
    for (let i=0;i<14;i++) {
      const x = ((i*.237+t*.012)%1.35-.15)*w;
      const y = h*(-.10+progress*.19)+Math.sin(t*.31+i*1.7)*h*.07;
      const r = w*(.15+(i%3)*.045);
      const g=paint.createRadialGradient(x,y,r*.06,x,y,r);
      g.addColorStop(0,`rgba(30,29,28,${.15+progress*.31})`);
      g.addColorStop(.48,`rgba(75,69,60,${.07+progress*.13})`);
      g.addColorStop(1,'rgba(38,35,31,0)');
      paint.fillStyle=g;paint.fillRect(x-r,y-r,r*2,r*2);
    }
    // Soft overlapping hot-gas particles: irregular tongues rather than a row of cutout flames.
    paint.globalCompositeOperation='screen';
    paint.filter='blur(2px)';
    for(let i=0;i<85;i++) {
      const seed=(Math.sin(i*127.1+17)*43758.5453)%1;
      const life=(t*(.55+(i%7)*.075)+i*.618033)%1;
      const center=((i*.381966)%1)*w;
      const rise=h*(.04+heat*(.13+.07*Math.abs(seed)));
      const x=center+Math.sin(t*2+i*3)*w*.012*life;
      const y=h+8-life*rise;
      const rx=w*(.008+.012*(1-life))*(.7+Math.abs(seed));
      const ry=h*(.025+.04*(1-life));
      const alpha=(1-life)*(1-life)*(.06+progress*.29);
      paint.save();paint.translate(x,y);paint.scale(rx,ry);
      const g=paint.createRadialGradient(0,0,0,0,0,1);
      g.addColorStop(0,`rgba(255,221,129,${alpha})`);
      g.addColorStop(.25,`rgba(255,146,37,${alpha*.85})`);
      g.addColorStop(.58,`rgba(238,58,9,${alpha*.55})`);
      g.addColorStop(1,'rgba(177,31,4,0)');
      paint.fillStyle=g;paint.fillRect(-1,-1,2,2);paint.restore();
    }
    paint.filter='none';
    const emberCount=Math.floor(14+progress*90);
    for(let i=0;i<emberCount;i++) {
      const speed=.12+(i%7)*.013+progress*.11;
      const phase=(t*speed+i*.618)%1;
      const x=((i*.381966)%1)*w+Math.sin(t*.8+i*2)*w*.025;
      const y=h*(1-phase), radius=.7+(i%3)*.6;
      paint.globalAlpha=(1-phase)*(.23+progress*.7);
      paint.fillStyle=i%4?'#f78636':'#ffe9a8';
      paint.beginPath();paint.ellipse(x,y,radius,radius*(1.8+progress*1.3),.3,0,Math.PI*2);paint.fill();
      if(i%6===0){paint.globalAlpha*=.25;paint.beginPath();paint.arc(x,y,radius*3,0,Math.PI*2);paint.fill();}
    }
    paint.globalAlpha=1;
  }
  requestAnimationFrame(atmosphere);
