...
function weightedPick(weights){
  const r=Math.random()*Object.values(weights).reduce((sum,w)=>sum+w,0); let acc=0;
...
function routeCenters(){const step=Math.min(180,State.w*.165);const c=State.w/2;return [-2,-1,0,1,2].map(i=>c+i*step);}
...
function update(dt){
 ...
 const riverSpeed=(135+progress*35)*(1+Math.min(.18,imbalance*.004+State.pressure*.10))*State.riverMultiplier;
 ...
 if(p.ground&&p.on)p.x=clamp(p.x+p.on.x-(p.on.previousX??p.on.x),8,State.w-p.w-8);
 ...
 if(p.ground&&p.on){
   if(p.on.retired){p.ground=false;p.on=null;p.coyote=.14;}
   const b=p.on?surfaceBounds(p.on):null;
   if(!b||p.x+p.w<=b.left+3||p.x>=b.right-3){p.ground=false;p.on=null;p.coyote=.14;}
 ...
 if(!p.ground&&State.endMode!=='win'){
   ...
   if(p.vy>=0){
     ...
     for(const q of State.platforms){
       if(q.retired||!q.active||q.kind==='start')continue;
       ...
       if(prevBottom<=(q.previousSurfaceY??b.y)+3&&nowBottom>=b.y-3&&b.y<hitY){hit=q;hitY=b.y;}
     }
     if(hit){
 ...
 // First-landing credit persists for the whole run.
 if(!State.endMode&&State.startRetired)for(const q of State.platforms){
   if(!q.active||!q.reward||q.reward.collected||q.reward.type==='none'||q.retired)continue;
   const rx=q.x+q.w/2+(q.rewardOffsetX||0);
   const ry=surfaceBounds(q).y-Math.max(q.rewardOffsetY||30,44)+Math.sin(State.t*2.5+q.spark)*2.2;
   const radius=q.reward.tier>=2?28:23;
   const nx=clamp(rx,p.x+8,p.x+p.w-8),ny=clamp(ry,p.y+12,p.y+p.h);
   if((rx-nx)**2+(ry-ny)**2>radius*radius)continue;
   q.reward.collected=true;
   const tier=q.reward.tier;
   State.pickupPulse=.65;
   if(q.reward.type==='value'){const gain=tier+(tier>=2&&State.flow>=2?1:0);State.value+=gain;showGain(rx,ry,'+'+gain+' VALUE');State.flow=clamp(State.flow+.05*tier,1,3);State.pressure=Math.max(0,State.pressure-.06*tier);}
   else {State.attention+=tier;showGain(rx,ry,'+'+tier+' ATTENTION','blue');State.pressure=clamp(State.pressure+.12*tier,0,1);State.flow=clamp(State.flow-.04*tier,1,3);}
 ...
 const camTarget=p.y-State.h*.46;
 // Hold the view while being carried; follow downhill only near the lower edge.
 if(camTarget<State.camera||worldY(p.y+p.h)>State.h*.82)State.camera+=(camTarget-State.camera)*Math.min(1,dt*5);
 ...
 if(State.t>3.5)$('#toast').classList.add('hide');updateHUD();
}
function updateHUD(){
  const p=State.player||{y:State.startY,h:0};
  const progress=clamp((State.startY-p.y-p.h)/(State.startY-State.stageY),0,1);
...
function drawPlatformReward(q){
 ...
 if(q.retired||!q.reward||q.reward.collected||q.reward.type==='none'||q.spec.name==='attentionBall') return;
 ...
 if(q.reward.tier>=3){ctx.font='900 12px Arial';ctx.textAlign='center';ctx.fillStyle='#ffe5a3';ctx.fillText('+'+q.reward.tier,0,22);}
