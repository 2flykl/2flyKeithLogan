...
// Mid-ground detail breaks up the old primitive-shape feel: worn path, stones, shrubs and fence remnants.
const pathCurve=new THREE.CatmullRomCurve3([new THREE.Vector3(2,.035,18),new THREE.Vector3(-3,.04,-4),new THREE.Vector3(5,.035,-30),new THREE.Vector3(-8,.035,-63),new THREE.Vector3(2,.035,-98)]);
const path=new THREE.Mesh(new THREE.TubeGeometry(pathCurve,90,2.15,10,false),new THREE.MeshStandardMaterial({color:0xb69a67,roughness:1,transparent:true,opacity:.48}));path.scale.y=.018;path.receiveShadow=true;scene.add(path);
const rockMat=new THREE.MeshStandardMaterial({color:0x74786d,roughness:1});
for(let i=0;i<42;i++){const rock=new THREE.Mesh(new THREE.DodecahedronGeometry(.28+Math.random()*.72,0),rockMat);rock.scale.set(1+.7*Math.random(),.55+.5*Math.random(),.75+.6*Math.random());const side=i%2?1:-1;rock.position.set(side*(5+Math.random()*28),.18,-3-Math.random()*100);rock.rotation.set(Math.random(),Math.random()*Math.PI,Math.random()*.4);rock.castShadow=true;rock.receiveShadow=true;scene.add(rock)}
const shrubMat=new THREE.MeshStandardMaterial({color:0x3f713f,roughness:1});
for(let i=0;i<34;i++){const bush=new THREE.Group();for(let j=0;j<3;j++){const b=new THREE.Mesh(new THREE.IcosahedronGeometry(.55+Math.random()*.45,1),shrubMat);b.position.set((j-1)*.45+Math.random()*.25,.42+Math.random()*.25,(Math.random()-.5)*.35);b.scale.y=.72;b.castShadow=true;bush.add(b)}const side=i%2?1:-1;bush.position.set(side*(9+Math.random()*42),0,-6-Math.random()*115);scene.add(bush)}
const fenceMat=new THREE.MeshStandardMaterial({color:0x76583a,roughness:1});
for(const side of[-1,1]){for(let i=0;i<8;i++){const post=new THREE.Mesh(new THREE.CylinderGeometry(.075,.1,1.9,6),fenceMat);post.position.set(side*(13+i*.35),.93,-18-i*8.5);post.rotation.z=side*(.03+(i%3)*.015);post.castShadow=true;scene.add(post)}}
...
function human({shirt,pants,skin,cap=false,hoodie=false}){
  // Deliberately leaner proportions: less blocky/Roblox-like while staying lightweight procedural 3D.
  const g=new THREE.Group(),ms=new THREE.MeshStandardMaterial({color:skin,roughness:.82}),mt=new THREE.MeshStandardMaterial({color:shirt,roughness:.9}),mp=new THREE.MeshStandardMaterial({color:pants,roughness:.92}),shoe=new THREE.MeshStandardMaterial({color:0x111318,roughness:.75});
  const torso=new THREE.Mesh(new THREE.CapsuleGeometry(.49,1.78,7,12),mt);torso.position.y=3.78;torso.scale.set(.92,1,.54);torso.castShadow=true;g.add(torso);
  const head=new THREE.Mesh(new THREE.SphereGeometry(.47,20,16),ms);head.position.y=5.36;head.scale.set(.94,1.05,.92);head.castShadow=true;g.add(head);
  const neck=new THREE.Mesh(new THREE.CylinderGeometry(.18,.20,.34,10),ms);neck.position.y=4.92;neck.castShadow=true;g.add(neck);
  if(cap){const brim=new THREE.Mesh(new THREE.BoxGeometry(.69,.065,.5),new THREE.MeshStandardMaterial({color:0x08090b}));brim.position.set(0,5.72,-.23);g.add(brim);const crown=new THREE.Mesh(new THREE.SphereGeometry(.49,18,8,0,Math.PI*2,0,Math.PI/2),new THREE.MeshStandardMaterial({color:0x07090a}));crown.position.y=5.61;g.add(crown)}
  if(hoodie){const h=new THREE.Mesh(new THREE.TorusGeometry(.44,.105,8,20,Math.PI*1.35),mt);h.position.set(0,5.12,.18);h.rotation.x=1.3;g.add(h)}
  const limbs={};
  ['lArm','rArm'].forEach((k,i)=>{const p=new THREE.Group(),a=new THREE.Mesh(new THREE.CapsuleGeometry(.125,1.38,5,9),ms);a.position.y=-.71;a.scale.z=.86;a.castShadow=true;p.add(a);p.position.set(i?-.67:.67,4.39,0);g.add(p);limbs[k]=p});
  ['lLeg','rLeg'].forEach((k,i)=>{const p=new THREE.Group(),l=new THREE.Mesh(new THREE.CapsuleGeometry(.175,1.86,5,9),mp);l.position.y=-.96;l.scale.z=.88;l.castShadow=true;p.add(l);const sh=new THREE.Mesh(new THREE.BoxGeometry(.36,.20,.68),shoe);sh.position.set(0,-2.02,-.15);sh.rotation.x=-.06;p.add(sh);p.position.set(i?-.29:.29,2.16,0);g.add(p);limbs[k]=p});
...
// One world-space catch target. It is positioned ONCE at launch and never follows the player.
const zoneMesh=new THREE.Group();
const zoneRing=new THREE.Mesh(new THREE.RingGeometry(1.72,2.48,64),new THREE.MeshBasicMaterial({color:0x70ff9a,transparent:true,opacity:.58,side:THREE.DoubleSide,depthWrite:false}));zoneRing.rotation.x=-Math.PI/2;zoneRing.position.y=.045;zoneMesh.add(zoneRing);
const zoneInner=new THREE.Mesh(new THREE.CircleGeometry(1.7,64),new THREE.MeshBasicMaterial({color:0x70ff9a,transparent:true,opacity:.075,side:THREE.DoubleSide,depthWrite:false}));zoneInner.rotation.x=-Math.PI/2;zoneInner.position.y=.041;zoneMesh.add(zoneInner);
const zoneBeacon=new THREE.Mesh(new THREE.CylinderGeometry(.045,.045,2.4,8),new THREE.MeshBasicMaterial({color:0xd8ffe3,transparent:true,opacity:.24,depthWrite:false}));zoneBeacon.position.y=1.2;zoneMesh.add(zoneBeacon);
zoneMesh.position.set(1.3,0,6.7);zoneMesh.visible=false;scene.add(zoneMesh);
