...
import {addOverlookDetail} from './environment-detail.js';
const TAU=Math.PI*2;
...
export class World{
 ...
 async load(onProgress){
  ...
  this.overlookDetail=addOverlookDetail(this.scene,this.stageCenter,this.compactTexture);
 ...
 buildGroundStage(){
  ...
  for(let i=0;i<pos.count;i++){
   ...
   const n=Math.sin(x*.7)*Math.cos(z*.6)*.045+Math.sin(x*1.7+z*.8)*.012;
   pos.setY(i,n*edge);
  }
  pos.needsUpdate=true;
  groundGeo.computeVertexNormals();
  const groundMat=new THREE.MeshStandardMaterial({map:diffuse,roughness:.98,metalness:0,color:0xffffff,transparent:true,depthWrite:false});
  this.ground=new THREE.Mesh(groundGeo,groundMat);
  this.ground.position.set(this.stageCenter.x,-.03,this.stageCenter.z);
  this.ground.renderOrder=-20;this.scene.add(this.ground);
  ...
  this.groundSkirt.renderOrder=-21;this.scene.add(this.groundSkirt);
 }
 makeGroundTexture(size){
  ...
  // Fine dry-grass strokes and soil grain replace broad painted-looking blobs.
  for(let i=0;i<46000;i++){
   const x=hash(i,7)*size,y=hash(i,19)*size;
   ctx.strokeStyle=i%3===0?'rgba(194,176,117,.24)':'rgba(44,48,28,.17)';ctx.lineWidth=.5+hash(i,37);
   ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+(hash(i,11)-.5)*4,y-2-hash(i,23)*7);ctx.stroke();
  }
  const feather=ctx.createRadialGradient(size/2,size/2,size*.25,size/2,size/2,size*.51);feather.addColorStop(0,'white');feather.addColorStop(.7,'rgba(255,255,255,.7)');feather.addColorStop(1,'rgba(255,255,255,0)');ctx.globalCompositeOperation='destination-in';ctx.fillStyle=feather;ctx.fillRect(0,0,size,size);
 ...
 update(dt,g,{intro=false,exploring=false,reduced=false,cutaway=false,frozen=false}={}){
  ...
  this.overlookDetail.update(this.clock,reduced);
  ...
  const moving=Math.hypot(g.playerX-(this.lastPlayerX??g.playerX),g.playerZ-(this.lastPlayerZ??g.playerZ))>.002;
  this.lastPlayerZ=g.playerZ;
  ...
  if(caught&&g.catchGrade==='trick')this.catchStyle='jump_catch';
  if(caught&&!cutaway&&!frozen)this.catchAge+=dt;
  if(!caught)this.catchAge=0;
  let variant=null;
  if(g.phase==='ready'&&this.readyStyle==='tracking')variant='one_hand';
  if(g.phase==='result'&&!caught&&g.phaseTime>.7&&this.readyStyle==='tracking')variant='tracking';
  ...
  const photoDir=this.direction(this.documentarian),direction=DIRECTIONS[photoDir],animated=this.photoTextures[direction]||(!['idle','ready','walk'].includes(this.photographer.state)?this.photoTextures['front-right']:null);
