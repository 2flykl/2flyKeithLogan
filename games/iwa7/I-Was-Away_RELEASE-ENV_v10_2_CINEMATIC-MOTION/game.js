...
TX.grass=loadEnvGround('ground_grass_lush.jpg',11,11);
...
// ---- V10.2 CINEMATIC 360 DISTANT WORLD ----
// The panorama is kept far away while playable ground and vegetation remain real 3D.
// This preserves orbit parallax near the player without exposing flat image cards.
const cinematicPano=envTextureLoader.load('./environment_cinematic/panorama.png');
cinematicPano.colorSpace=THREE.SRGBColorSpace;
cinematicPano.wrapS=THREE.RepeatWrapping; cinematicPano.wrapT=THREE.ClampToEdgeWrapping;
cinematicPano.repeat.set(2,1);
cinematicPano.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());
const panoShell=new THREE.Mesh(
  new THREE.CylinderGeometry(235,235,104,128,1,true),
  new THREE.MeshBasicMaterial({map:cinematicPano,side:THREE.BackSide,fog:false,toneMapped:false})
);
panoShell.position.y=42;
panoShell.rotation.y=Math.PI*.18;
scene.add(panoShell);
...
// Cinematic playable overlook: irregular rocky foreground reads as real terrain against the panorama.
const overlook=new THREE.Mesh(
  new THREE.CircleGeometry(15.5,96),
  new THREE.MeshStandardMaterial({color:0x9b8f72,map:TX.stone,bumpMap:TX.stone,bumpScale:.19,roughness:.96})
);
overlook.rotation.x=-Math.PI/2; overlook.position.set(0,.055,5.5); overlook.receiveShadow=true; scene.add(overlook);
const overlookInner=new THREE.Mesh(
  new THREE.CircleGeometry(10.8,96),
  new THREE.MeshStandardMaterial({color:0x7e8b5e,map:TX.grass,bumpMap:TX.grass,bumpScale:.12,roughness:.98,transparent:true,opacity:.62})
);
overlookInner.rotation.x=-Math.PI/2; overlookInner.position.set(0,.069,5.5); overlookInner.receiveShadow=true; scene.add(overlookInner);
// warm atmospheric veil helps ground geometry disappear naturally into the distant panorama
const valleyVeilMat=new THREE.MeshBasicMaterial({color:0xf2c58c,transparent:true,opacity:.075,depthWrite:false,side:THREE.DoubleSide});
for(let i=0;i<4;i++){const v=new THREE.Mesh(new THREE.PlaneGeometry(190,18+i*5),valleyVeilMat.clone());v.position.set((i-1.5)*12,10+i*3,-92-i*24);v.material.opacity=.07-.009*i;scene.add(v);}
