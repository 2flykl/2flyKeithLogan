...
// ---- SKY / ATMOSPHERE ----
const skyMat=new THREE.ShaderMaterial({side:THREE.BackSide,uniforms:{top:{value:new THREE.Color(0x536b98)},mid:{value:new THREE.Color(0xc47767)},horizon:{value:new THREE.Color(0xf5bd73)},low:{value:new THREE.Color(0xe49a58)}},vertexShader:`varying vec3 w;void main(){w=(modelMatrix*vec4(position,1.)).xyz;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,fragmentShader:`uniform vec3 top,mid,horizon,low;varying vec3 w;void main(){float h=normalize(w).y;vec3 c=mix(low,horizon,smoothstep(-.12,.05,h));c=mix(c,mid,smoothstep(.02,.34,h));c=mix(c,top,smoothstep(.28,.88,h));gl_FragColor=vec4(c,1.);}`});
scene.add(new THREE.Mesh(new THREE.SphereGeometry(420,40,22),skyMat));
// ---- V10.3 WORLD BLEND: SEAMLESS DISTANT WORLD + SKY ----
// A mirrored seamless panorama is feathered at the top/bottom, so the sky dome and 3D terrain
// show through instead of exposing a hard rectangular image edge.
...
cinematicPano.wrapS=THREE.ClampToEdgeWrapping; cinematicPano.wrapT=THREE.ClampToEdgeWrapping;
cinematicPano.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());
const panoShell=new THREE.Mesh(
  new THREE.CylinderGeometry(248,248,112,160,1,true),
  new THREE.MeshBasicMaterial({map:cinematicPano,side:THREE.BackSide,fog:false,toneMapped:false,transparent:true,depthWrite:false,opacity:.97})
);
panoShell.position.y=34;
panoShell.rotation.y=Math.PI*.125;
scene.add(panoShell);
// Horizon veil: hides the final perceptual join between distant photography and real 3D terrain.
const horizonVeil=new THREE.Mesh(new THREE.CylinderGeometry(222,222,32,128,1,true),new THREE.MeshBasicMaterial({color:0xe7bb86,transparent:true,opacity:.085,side:THREE.BackSide,depthWrite:false,fog:false}));
horizonVeil.position.y=8;scene.add(horizonVeil);
...
const ground=mesh(groundGeo,new THREE.MeshStandardMaterial({color:0x8a7c58,map:TX.grass,bumpMap:TX.grass,bumpScale:.16,roughness:.98}),false,true);ground.rotation.x=-Math.PI/2;scene.add(ground);
...
const path=mesh(new THREE.TubeGeometry(pathCurve,72,1.35,12,false),new THREE.MeshStandardMaterial({color:0x8f7152,map:TX.dirt,bumpMap:TX.dirt,bumpScale:.08,roughness:1,transparent:true,opacity:.72}),false,true);path.scale.y=.012;scene.add(path);
...
const overlook=new THREE.Mesh(irregularPatch(16.7,84,2.4),new THREE.MeshStandardMaterial({color:0x86765c,map:TX.stone,bumpMap:TX.stone,bumpScale:.12,roughness:.99,transparent:true,opacity:.46}));
overlook.rotation.x=-Math.PI/2;overlook.position.set(0,.075,5.5);overlook.receiveShadow=true;scene.add(overlook);
const overlookInner=new THREE.Mesh(irregularPatch(11.2,72,4.6),new THREE.MeshStandardMaterial({color:0x7f805e,map:TX.grass,bumpMap:TX.grass,bumpScale:.08,roughness:.99,transparent:true,opacity:.34}));
