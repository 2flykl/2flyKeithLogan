...
const scene=new THREE.Scene();scene.background=new THREE.Color(0xa9c3d0);scene.fog=new THREE.FogExp2(0xb7b69d,.0055);
...
TX.grass=loadEnvGround('ground_meadow_mix.jpg',18,18);
TX.dirt=loadEnvGround('ground_path_dirt.jpg',5,16);
TX.stone=loadEnvGround('ground_rocky.jpg',3,3);
...
// ---- SKY / ATMOSPHERE ----
const skyMat=new THREE.ShaderMaterial({side:THREE.BackSide,uniforms:{top:{value:new THREE.Color(0x87b4ce)},horizon:{value:new THREE.Color(0xd9c2a2)},low:{value:new THREE.Color(0xf1d2aa)}},vertexShader:`varying vec3 w;void main(){w=(modelMatrix*vec4(position,1.)).xyz;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,fragmentShader:`uniform vec3 top,horizon,low;varying vec3 w;void main(){float h=normalize(w).y;vec3 c=mix(low,horizon,smoothstep(-.08,.16,h));c=mix(c,top,smoothstep(.05,.75,h));gl_FragColor=vec4(c,1.);}`});
scene.add(new THREE.Mesh(new THREE.SphereGeometry(420,40,22),skyMat));
...
const ridgeBands=[];for(let i=0;i<5;i++){const ridge=new THREE.Mesh(new THREE.PlaneGeometry(260,14+8*i,90,1),new THREE.MeshBasicMaterial({color:[0x6d7d65,0x7d8f70,0x95a17a,0xafb287,0xc4bd91][i],transparent:true,opacity:.34-.04*i,depthWrite:false}));ridge.position.set((i-2)*8,3.2+i*1.6,-150-i*16);ridge.rotation.x=-0.01*i;ridgeBands.push(ridge);scene.add(ridge)}


// ---- TERRAIN ----
const groundGeo=new THREE.PlaneGeometry(360,360,150,150);const pa=groundGeo.attributes.position;
for(let i=0;i<pa.count;i++){const x=pa.getX(i),y=pa.getY(i);const z=Math.sin(x*.028)*1.0+Math.cos(y*.025)*.72+Math.sin((x-y)*.015)*.65+Math.sin(Math.hypot(x,y)*.022)*.34;pa.setZ(i,z)}
pa.needsUpdate=true;groundGeo.computeVertexNormals();
const ground=mesh(groundGeo,new THREE.MeshStandardMaterial({color:0x94a764,map:TX.grass,bumpMap:TX.grass,bumpScale:.22,roughness:.96}),false,true);ground.rotation.x=-Math.PI/2;scene.add(ground);

// dirt footpath
const pathCurve=new THREE.CatmullRomCurve3([new THREE.Vector3(1,.06,18),new THREE.Vector3(-1,.08,4),new THREE.Vector3(3,.06,-16),new THREE.Vector3(-5,.07,-38),new THREE.Vector3(5,.05,-72),new THREE.Vector3(-1,.05,-110)]);
const path=mesh(new THREE.TubeGeometry(pathCurve,110,2.45,14,false),new THREE.MeshStandardMaterial({color:0xb6976d,map:TX.dirt,bumpMap:TX.dirt,bumpScale:.11,roughness:1}),false,true);path.scale.y=.015;scene.add(path);
...
function envHeight(x,z){const y=-z;return Math.sin(x*.028)*1.0+Math.cos(y*.025)*.72+Math.sin((x-y)*.015)*.65+Math.sin(Math.hypot(x,y)*.022)*.34;}
...
function crossTree(x,z,s=1,variant=0,rot=0){
  const g=new THREE.Group();const mat=new THREE.MeshBasicMaterial({map:ENV_TREE[variant%ENV_TREE.length],transparent:true,alphaTest:.08,side:THREE.DoubleSide,depthWrite:true,toneMapped:false});
...
function envSprite(texture,x,z,w,h,opacity=1,rot=0){
  const mat=new THREE.SpriteMaterial({map:texture,transparent:true,alphaTest:.055,depthWrite:true,depthTest:true,toneMapped:false,opacity});
...
// Tonal meadow variation breaks up repeated ground tiling.
const meadowPatchMat=[0x71854d,0xa39c5b,0x667d45,0x8f9c5e].map(c=>new THREE.MeshBasicMaterial({color:c,transparent:true,opacity:.045,depthWrite:false,side:THREE.DoubleSide}));
for(let i=0;i<24;i++){const rr=9+envRand()*22;const a=envRand()*Math.PI*2,r=12+envRand()*105;const x=Math.cos(a)*r,z=Math.sin(a)*r-32;const p=new THREE.Mesh(new THREE.CircleGeometry(rr,42),meadowPatchMat[i%meadowPatchMat.length]);p.position.set(x,envHeight(x,z)+.02,z);p.rotation.x=-Math.PI/2;p.scale.y=.52+envRand()*.5;scene.add(p)}
...
function terrainHeightAt(x,z){
  const y=-z;
  return Math.sin(x*.028)*1.0+Math.cos(y*.025)*.72+Math.sin((x-y)*.015)*.65+Math.sin(Math.hypot(x,y)*.022)*.34;
}
...
const BRIDGE_TX={};
for(const stateName of ['throw_accel','release_recovery','catch_approach','catch_absorb']){
  BRIDGE_TX[stateName]={};
...
function planGuideAvoidance(){
  ...
  const dx=guide.position.x-state.catchPoint.x;
  const dz=guide.position.z-state.catchPoint.z;
  const dist=Math.hypot(dx,dz);
  if(dist<4.2){
    const nx=dist<.001?-1:dx/dist;
    const nz=dist<.001?0:dz/dist;
    guideMoveTarget.set(
      THREE.MathUtils.clamp(state.catchPoint.x+nx*5.1,-15.5,15.5),
      0,
      THREE.MathUtils.clamp(state.catchPoint.z+nz*4.3,4.8,15.8)
    );
    guideAvoidActive=true;
  }
}
function updateGuidePosition(dt){
  const speed=guideAvoidActive?4.4:2.2;
  guide.position.x=THREE.MathUtils.lerp(guide.position.x,guideMoveTarget.x,Math.min(1,dt*speed));
  guide.position.z=THREE.MathUtils.lerp(guide.position.z,guideMoveTarget.z,Math.min(1,dt*speed));
...
const state={phase:'idle',power:0,powerDir:1,aim:0,timing:0,releaseQuality:0,flightT:0,flightDuration:5.8,start:new THREE.Vector3(),catchPoint:new THREE.Vector3(),catchT:0,catchPressed:false,catchResult:'',demoT:0};const keys={};let viewMode=0,orbit=0,orbitTarget=0,orbitPitch=.27,orbitPitchTarget=.27,orbitZoom=1,orbitZoomTarget=1;let orbitDragging=false,orbitPointer=-1,lastOrbitX=0,lastOrbitY=0;
let cameraPunch=0;
...
function animatePerson(dt){
  ...
  if(state.phase==='flight'||state.phase==='catch'){
    ...
    player.userData.facing=normAng(orbit+Math.PI);
  }else if(state.phase==='demo'){faceTarget(player,guide.position)}
  else if(moving){player.userData.facing=normAng(orbit+Math.PI)}else{faceTarget(player,guide.position)}
  faceTarget(guide,player.position);

  let pState='idle_hold',pFrame=0;
  const u=state.flightDuration?Math.min(1,state.flightT/state.flightDuration):0;
  if(state.phase==='power')pState='windup_hold';
  else if(state.phase==='release'){
    // As the release window approaches, unwind through two generated acceleration bridges.
    if(state.timing<.48)pState='windup_hold';
    else {pState='throw_accel';pFrame=state.timing<.72?0:1;}
  }else if(state.phase==='flight'){
    if(u<.045){pState='release_recovery';pFrame=0;}
    else if(u<.09){pState='release_recovery';pFrame=1;}
    else if(u<.18)pState='release_empty';
    else if(u<.70)pState='empty_wait';
    else if(u<.79){pState='catch_approach';pFrame=0;}
    else if(u<.88){pState='catch_approach';pFrame=1;}
    else pState='catch_ready_empty';
  }else if(state.phase==='catch'){
    if(u<.93)pState='catch_ready_empty';
    else if(u<.958){pState='catch_absorb';pFrame=0;}
    else if(u<.982){pState='catch_absorb';pFrame=1;}
    else pState='catch_contact_hold';
  ...
  if(state.phase==='demo'){
    ...
    else if(t<2.63){gState='throw_accel';gFrame=0;}
    else if(t<2.80){gState='throw_accel';gFrame=1;}
    else if(t<2.98){gState='release_recovery';gFrame=0;}
    else if(t<3.18){gState='release_recovery';gFrame=1;}
    else if(t<4.25)gState='release_empty';
    else if(t<5.75)gState='empty_wait';
    else if(t<6.15){gState='catch_approach';gFrame=0;}
    else if(t<6.55){gState='catch_approach';gFrame=1;}
    else if(t<6.78){gState='catch_absorb';gFrame=0;}
    else if(t<6.95){gState='catch_absorb';gFrame=1;}
    else gState='catch_chest_hold';
  }
  setActorFrame(guide,gState,gFrame);

  const bob=moving?Math.sin(elapsed*10)*.035:Math.sin(elapsed*2.2)*.014;
  player.userData.sprite.position.y=.02+bob;guide.userData.sprite.position.y=.02+Math.sin(elapsed*1.8+.7)*.010;
}
