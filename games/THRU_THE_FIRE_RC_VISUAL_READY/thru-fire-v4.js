(() => {
  ...
  function grab() {
    ...
    const collectionAnchor = Array.isArray(currentRawObject()) && focusedItemIndex === 1 ? els.itemAnchorB : els.itemAnchor;
    collectionAnchor.classList.add("collected");
    els.grabBtn.disabled = true;

    window.setTimeout(() => {
      entry.status = "saved";
      collectionAnchor.classList.remove("collected");
  ...
  async function startAudio() {
    if (!audioEnabled) return;
    els.music.volume = .32;
    try { els.music.play().catch(() => {}); } catch (_) { /* Audio must never block entry. */ }
  ...
  function createFireAudio() {
    ...
    return {
      ...
      intensity: (level) => {
        gain.gain.setTargetAtTime(.018 + level * .026, context.currentTime, .4);
        filter.frequency.setTargetAtTime(420 + level * 480, context.currentTime, .4);
      }
    };
  }
