...
export class StreamsSystem {
  group; planetMesh; children = []; labelContainer; time = 0; objectData; onObjectClick = null; clickTargets = [];
  ...
  buildPlanet() {
    ...
    const mat = new THREE.ShaderMaterial({ uniforms: { time: { value: 0 }, deepColor: { value: new THREE.Color(0x0b1218) }, shallowColor: { value: new THREE.Color(0x6f5238) }, rimColor: { value: new THREE.Color(0xd7b38b) } }, vertexShader: `varying vec3 vNormal; varying vec3 vPos; uniform float time; void main(){ vNormal = normalize(normalMatrix * normal); vPos = position; vec3 displaced = position + normal * (12.0 * sin(position.y * 0.008 + time * 0.9) * cos(position.x * 0.006 + time * 0.55)); gl_Position = projectionMatrix * modelViewMatrix * vec4(displaced,1.0); }`, fragmentShader: `uniform vec3 deepColor; uniform vec3 shallowColor; uniform vec3 rimColor; uniform float time; varying vec3 vNormal; varying vec3 vPos; void main(){ vec3 viewDir = normalize(cameraPosition - vPos); float rim = pow(1.0 - max(0.0, dot(vNormal, viewDir)), 2.4); float wave = 0.5 + 0.5 * sin(vPos.y * 0.012 + vPos.x * 0.008 + time * 0.75); vec3 base = mix(deepColor, shallowColor, wave); gl_FragColor = vec4(mix(base, rimColor, rim * 0.55),1.0); }` });
  ...
  buildRecordOrbits() {
    [920, 1220, 1560, 1940, 2360, 2800].forEach((r) => {
      const ring = new THREE.Mesh(new THREE.RingGeometry(r - 2, r + 2, 160), new THREE.MeshBasicMaterial({ color: 0xf3f7ff, transparent: true, opacity: 0.095, side: THREE.DoubleSide, depthWrite: false }));
  ...
  update(dt, camera, renderer) {
    ...
    for (const child of this.children) {
      ...
      child.mesh.position.set(Math.cos(child.orbitAngle) * child.orbitRadius, child.orbitY + Math.sin(this.time * 0.75 + child.bobPhase) * 20, Math.sin(child.orbitAngle) * child.orbitRadius);
  ...
  updateLabels(camera, renderer) {
    ...
    for (const child of this.children) {
      ...
      const opacity = 1 - Math.min(1, Math.max(0, (dist - 900) / 4100));
      const ndc = worldPos.clone().project(camera);
      const x = (ndc.x * 0.5 + 0.5) * width;
      const y = (-(ndc.y * 0.5) + 0.5) * height;
      if (ndc.z > 1 || opacity < 0.02) child.labelEl.style.opacity = '0';
      else { child.labelEl.style.opacity = String(opacity); child.labelEl.style.left = `${x}px`; child.labelEl.style.top = `${y}px`; }
    }
  }
