...
function currentSettings(){
  ...
  return {
    ...
    attentionChance:0.12 + progress * 0.2
  };
}
...
function updatePlatforms(deltaSeconds,settings){
  for(const platform of state.platforms){
    ...
    if(!platform.anchored && !platform.isStage){
      platform.y += settings.speed*platform.flowFactor*deltaSeconds;
      platform.x += platform.lateralVelocity*deltaSeconds;
...
function drawPlatform(platform){
  ...
  ctx.strokeStyle=
    platform.isStage
      ? 'rgba(255,210,128,.9)'
      : 'rgba(154,232,242,.28)';
  ctx.lineWidth=platform.isStage?3:1.25;
  ctx.beginPath();
  ctx.ellipse(
    0,
    platform.height*.26,
