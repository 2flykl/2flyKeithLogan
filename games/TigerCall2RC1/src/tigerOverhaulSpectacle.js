...
let video, audio, canvas, ctx, overlay, leftRail, rightRail, floor, label, tigerSym;
let nextMarker=0, particles=[], flashes=[], stripeLevel=0, ultra=false, activeName='';

function init() {
  video=$('performanceVideo'); audio=$('gameplayAudio'); canvas=$('spectacleCanvas'); overlay=$('tigerSpectacle');
...
function syncToTime(){ nextMarker=0; stripeLevel=0; ultra=false; overlay.dataset.ultra='0'; overlay.dataset.party='0'; while(nextMarker<MARKERS.length && MARKERS[nextMarker].time <= audio.currentTime){ applyMarker(MARKERS[nextMarker],true); nextMarker++; } }
...
function draw(){ if(!ctx)return; ctx.clearRect(0,0,innerWidth,innerHeight); for(let i=particles.length-1;i>=0;i--){ const p=particles[i]; p.x+=p.vx;p.y+=p.vy;p.vy+=.05;p.life-=.018; if(p.life<=0){particles.splice(i,1);continue;} ctx.globalAlpha=Math.min(1,p.life); ctx.fillStyle=p.style==='white'?'#fff':'#ff6200'; ctx.beginPath();ctx.arc(p.x,p.y,p.size,0,Math.PI*2);ctx.fill(); } ctx.globalAlpha=1;
 if(overlay?.dataset.party==='1'){ const t=audio?.currentTime||0; ctx.save(); ctx.globalAlpha=.38; ctx.strokeStyle='#ff6200'; ctx.lineWidth=3; for(let i=0;i<7;i++){ const y=120+i*70; const x1=90+Math.sin(t*2+i)*30; const x2=innerWidth-x1; ctx.beginPath();ctx.arc(x1,y,12+(i%3)*5,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(x2,y,12+(i%3)*5,0,Math.PI*2);ctx.stroke(); } ctx.restore(); } }
function loop(){ if(!audio || audio.paused || audio.ended) return; while(nextMarker<MARKERS.length && MARKERS[nextMarker].time <= audio.currentTime+.015){ applyMarker(MARKERS[nextMarker]); nextMarker++; } draw(); requestAnimationFrame(loop); }
